...
public class MonthlyView implements ViewEventCalendar {

	public ObservableList<Event> SelectEvent() {
            Connection conn = connectdb();
		 ObservableList<Event> list3 = FXCollections.observableArrayList();
        try { 
            PreparedStatement ps = conn.prepareStatement("select * from event where month(`date`)=month(now()) AND year(`date`) = year(now())");
